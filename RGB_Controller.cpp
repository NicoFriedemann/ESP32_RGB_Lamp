...
RGB_Controller::RGB_Controller(void (*debug_print_fncptr)(String, e_debug_level), void (*own_delay_fncptr)(int), int WATCHDOG_PIN_ESP32, 
	...
	int LED2_PIN_RED, int LED2_PIN_GREEN, int LED2_PIN_BLUE)
{
	...
	clear_I2C_outputs();
	debug_print("RGB_Controller::RGB_Controller - constructor", e_debug_level::dl_info);
}
...
void RGB_Controller::cyclic_tasks()
{
	...
	if (watchdog_state==e_watchdog_state::triggered) {
		debug_print("RGB_Controller::cyclic_tasks - watchog triggered -> wait 5s -> reset!", e_debug_level::dl_error);
...
{
	debug_print("RGB_Controller::refresh_I2C_watchdog - run", e_debug_level::dl_debug);
...
{
	debug_print("RGB_Controller::check_I2C_watchdog - run", e_debug_level::dl_debug);
	if (_watchdog_last_change <= (millis() - 1000)) {
		...
		if (state_watchdog_pin != _watchdog_state_check) {
			...
			debug_print("RGB_Controller::check_watchdog - ok", e_debug_level::dl_debug);
		}
		else {
			debug_print("RGB_Controller::check_watchdog - failed", e_debug_level::dl_debug);
...
{
	debug_print("RGB_Controller::clear_I2C_outputs - run", e_debug_level::dl_debug);
...
{
	debug_print("RGB_Controller::set_esp32_statusled_on - run", e_debug_level::dl_debug);
	digitalWrite(_builtin_led_esp32, HIGH);
}

void RGB_Controller::set_esp32_statusled_off()
{
	debug_print("RGB_Controller::set_esp32_statusled_off - run", e_debug_level::dl_debug);
	digitalWrite(_builtin_led_esp32, LOW);
}
...
void RGB_Controller::set_rgbled(int led_nmbr, int rgb_color[3])
{
	if (led_nmbr == 1) {
		...
		debug_print("RGB_Controller::set_rgbled - led number invalid", e_debug_level::dl_error);
	}
}
...
void RGB_Controller::set_program(e_prog_nmbr program_number,int rgb_colors_man[3], float hsv_colors_man[3])
{
	if (program_number != _last_prog_nmbr) {
		debug_print("RGB_Controller::set_program_auto - program=" + String(program_number), e_debug_level::dl_info);
		...
		case man_pc_hsv:
			...
			break;
		default:
			debug_print("RGB_Controller::set_program_auto - program_number not valid", e_debug_level::dl_error);
			break;
		}
	}
}
...
void RGB_Controller::set_program(e_prog_nmbr program_number)
{
	...
	{
		debug_print("RGB_Controller::set_program - run", e_debug_level::dl_info);
		int rgb[3];
		float hsv[3];
	...
	{
		debug_print("RGB_Controller::set_program - invalid program number", e_debug_level::dl_error);
	}
}

void RGB_Controller::auto_program1()
{
	debug_print("RGB_Controller::auto_program1 - run", e_debug_level::dl_info);
...
{
	debug_print("prog2 : run", e_debug_level::dl_info);
	int prog2_spd = 3000;
	float var = 0.0;
	int rgb_colors[3];
	while (true)
...
{
	debug_print("prog3 : run", e_debug_level::dl_info);
	int prog3_spd = 500;
	int rgb_colors[3];
	while (true)
	{
...
{
	debug_print("prog4 : run", e_debug_level::dl_info);
	int prog4_speed = 20;
	int rgb_colors[3];
	while (true)
	{
...
{
	debug_print("prog5 : run", e_debug_level::dl_info);
	int prog5_speed = 40;
	int time = 0;
	int hue = 0;
	int rgb_colors[3];
...
{
	debug_print("prog6 : run", e_debug_level::dl_info);
	int hue = 0;
	int rgb_colors[3];
	while (true)
	{
...
{
	debug_print("prog7 : run", e_debug_level::dl_info);
	int hue_offset;
	int prog7_speed = 20;
	int rgb_colors[3];
	while (true)
...
{
	debug_print("prog8 : run", e_debug_level::dl_info);
	int prog8_speed = 50;
	float h = 1.0;
	int rgb_colors[3];
	while (true)
...
{
	debug_print("prog9 : run", e_debug_level::dl_info);
	float min_val = 0.0;
	float max_val = 1.0;
	float hue = 0.0;
	int delay = 0;
...
{
	debug_print("prog_man_blynk : run", e_debug_level::dl_info);
	while (true)
	{
		set_rgbled(1, rgb_colors);
		set_rgbled(2, rgb_colors);
...
{
	debug_print("prog_man_pc_rgb : run", e_debug_level::dl_info);
	while (true)
	{
		set_rgbled(1, rgb_colors);
		set_rgbled(2, rgb_colors);
...
{
	debug_print("prog_man_pc_hsv : run", e_debug_level::dl_info);
	int rgb_colors[3];
	while (true)
	{
